...
def path_to_string(path: list, bop = None, eop = None) -> str:
    result = ""
    ...
    return result.strip() if bop is None and eop is None else bop + result.strip() + eop


def predict_relation_path_to_string(path: list) -> str:
    result = ""
    for i, p in enumerate(path):
        if i == 0:
            h, r, t = p
    ...
    return result.strip() + " " + "[<KG>]"


def predict_triple_path_to_string(path: list) -> str:
    result = ""
    for i, p in enumerate(path):
        if i == 0:
            h, r, t = p
...
class InstructFormater(object):
    ...
    def format(self, system, query):

        return self.prompt_template.format(system=system, query=query)
